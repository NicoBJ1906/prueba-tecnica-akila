...
def fila(**cambios):
    """Fila válida mínima; los tests sobrescriben solo lo que les interesa."""
    base = {
        "id": 1,
        "torre": "Torre 1",
        "piso": 5,
...
class TestValidacionEsquema:
    ...
    def test_vendido_sin_fecha_de_venta(self):
...
class TestConsolidacion:
    def test_unidad_sin_duplicados_se_conserva(self):
        df = df_de(fila(id=1))
        assert len(consolidar(df)) == 1

    def test_gana_la_fila_vendida_sobre_la_disponible(self):
        df = df_de(
    ...
    def test_el_id_alto_no_gana_a_una_venta(self):
    ...
    def test_entre_dos_ventas_gana_la_mas_reciente(self):
        df = df_de(
    ...
    def test_sin_ventas_gana_el_registro_mas_reciente(self):
        df = df_de(
    ...
    def test_empate_de_fecha_se_rompe_por_id(self):
    ...
    def test_unidades_distintas_no_se_mezclan(self):
        df = df_de(
    ...
    def test_es_determinista_ante_el_orden_de_lectura(self):
        filas = [
            fila(id=1, estado=ESTADO_DISPONIBLE),
            fila(id=2, estado=ESTADO_VENDIDO, fecha_venta=pd.Timestamp("2026-01-10")),
...
class TestInformeCalidad:
    def test_sin_duplicados_no_hay_conflictos(self):
    ...
    def test_detecta_campos_en_conflicto(self):
        df = df_de(
    ...
    def test_detecta_ventas_multiples_y_estado_contradictorio(self):
        df = df_de(
